...
const MINI_EMPLOYEES = [
  { name: 'Jexa HR', status: 'Screening candidates', color: '#C1121F' },
  { name: 'Jaan', status: 'Handling enquiries', color: '#3B82F6' },
]

const CHART_POINTS = '0,38 14,32 28,34 42,22 56,26 70,14 84,18 100,8'
const WAVEFORM_BARS = [6, 12, 8, 16, 10, 14, 7, 11, 5]

export function DashboardPreview() {
  return (
    <div className="float-anim relative select-none">
      {/* soft breathing glow behind the dashboard panel */}
      <div className="glow-pulse pointer-events-none absolute -inset-6 -z-10 rounded-[32px] bg-brand-600/15 blur-3xl" />

      <div className="overflow-hidden rounded-2xl border border-ink-200 bg-surface shadow-overlay">
        <div className="flex h-9 items-center gap-3.5 border-b border-ink-100 bg-ink-25 px-4">
          <LogoMark size={16} />
          <span className="text-[11px] font-semibold text-ink-700">JEXA.AI</span>
          ...
          <div className="flex-1 p-4">
            ...
            <div className="mt-3 grid grid-cols-4 gap-2">
              {MINI_KPIS.map((kpi, i) => (
                <div
                  key={kpi.label}
                  className="hero-reveal rounded-lg border border-ink-100 bg-surface-elevated p-2"
                ...
                <div className="mt-2 space-y-2">
                  {MINI_EMPLOYEES.map((emp) => (
                    <div key={emp.name} className="flex items-center gap-1.5">
                      ...
                      <span className="status-dot-live size-1.5 rounded-full bg-success-500" />
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="absolute -bottom-7 -right-6 w-44 rounded-xl border border-ink-200 bg-surface-elevated p-3 shadow-glow-red sm:-right-10 sm:w-48">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="flex items-center gap-1 text-[8.5px] text-success-600">
              <span className="status-dot-live size-1.5 rounded-full bg-success-500" />
              On Call
            </p>
          </div>
        ...
        <div className="mt-2.5 flex h-5 items-end gap-[3px]">
          {WAVEFORM_BARS.map((h, i) => (
            <span
              key={i}
              className="waveform-bar w-[3px] rounded-full bg-brand-600"
