...
//
// organizationService, hrService and voiceAgentBuilderService are wired to
// the real FastAPI backend (app/organizations, app/ai_employees/hr,
// app/ai_employees/voice) — see services/api/*.service.ts for the HTTP
// calls. Everything else here is still mock; swap the same way once a real
// backend module exists with a matching shape.
...
// Jaan (Voice AI Workforce console) — voiceAgentBuilderService above is now
// real (agent CRUD/versioning/publish + the provider/voice catalog +
// start-test-call); telephonyService/voiceService and everything below are
// still mock. The console also uses these Jaan-specific mock domains.
